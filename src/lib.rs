...
fn generate(
    ...
    // Additionally generate into_* and take_* methods per field once per field when generating getters
    // We hook this in only when processing Getter variants to avoid duplicates
    if matches!(fn_type, Fns::Getter(_)) && rules.gen_into {
        // Check if field type is a primitive/Copy type - skip into_* for those
        let is_primitive = match &field.ty {
            Type::Path(tp) => {
                if let Some(last) = tp.path.segments.last() {
                    PRIMITIVE_TYPES.contains(&last.ident.to_string().as_str())
                } else {
                    false
                }
            }
        ...
        // Skip into_* for primitive/Copy types (they already return by value)
        if is_primitive {
            return;
        }

        // Build method base name from field or index (respect alias for named fields)
        let base_name = match &field.ident {
            Some(ident) => rules
                .alias
                .as_ref()
                .cloned()
            ...
            None => {
                // tuple field: use alias if provided; otherwise use "nth_{idx}"
                if let Some(a) = rules.alias.as_ref() {
                    a.clone()
                } else {
                    Ident::new(&format!("nth_{}", idx), Span::call_site())
                }
            }
        };

        // Use custom prefix if provided, otherwise default to "into"
        let into_prefix = rules.into_prefix.as_deref().unwrap_or(INTO_PREFIX_DEFAULT);
        let into_name = Ident::new(&format!("{}_{}", into_prefix, base_name), Span::call_site());
        let take_name = Ident::new(&format!("take_{}", base_name), Span::call_site());

        let getter_visibility = rules.getter_visibility_token();
        let getter_inline = rules.getter_inline_token();

        // Special-casing based on field type to choose best take implementation and docs
        let extra = match &field.ty {
            // Path types like Option<T>, Vec<T>, String, Box<T>, HashMap<..>, etc.
            Type::Path(tp) => {
                if let Some(last) = tp.path.segments.last() {
                    let ident = &last.ident;
                    if ident == "Option" {
                        // extract inner type T for docs/return
                        if let PathArguments::AngleBracketed(args) = &last.arguments {
                            if let Some(inner) = args.args.first() {
                                quote! {
                                    #[doc = concat!(" Consumes `self` and returns the `", stringify!(#field_access), "` field.")]
                                    #[doc = ""]
                                    #[doc = " This method moves the owned value out of the struct without cloning."]
                                    #[doc = ""]
                                    #[doc = " # Returns"]
                                    #[doc = ""]
                                    #[doc = concat!(" Returns Option<", stringify!(#inner), "> containing the field value, or None if the field was None.")]
                                    #[doc = ""]
                                    #[doc = " # Example"]
                                    #[doc = ""]
                                    #[doc = " ```ignore"]
                                    #[doc = concat!(" let obj = ", stringify!(#struct_name), "::default().", stringify!(#setter_name), "(value);")]
                                    #[doc = concat!(" let value = obj.", stringify!(#into_name), "();")]
                                    #[doc = " ```"]
                                    #[doc = ""]
                                    #[doc = " ---"]
                                    #[doc = " *Generated by `aksr` - Builder pattern macro*"]
                                    #getter_inline
                                    #getter_visibility fn #into_name(self) -> Option<#inner> {
                                        self.#field_access
                                    }

                                    #[doc = concat!(" Takes the `", stringify!(#field_access), "` field, leaving `None` in its place.")]
                                    #[doc = ""]
                                    #[doc = " This method moves the value out and replaces it with `None`, allowing you to continue using the struct."]
                                    #[doc = ""]
                                    #[doc = " # Returns"]
                                    #[doc = ""]
                                    #[doc = concat!(" Returns Option<", stringify!(#inner), "> containing the field value, or None if the field was None.")]
                                    ...
                                    #[doc = " # Note"]
                                    #[doc = ""]
                                    #[doc = " This does not require the inner type to implement `Default`."]
                                    #[doc = ""]
                                    #[doc = " # Example"]
                                    #[doc = ""]
                                    #[doc = " ```ignore"]
                                    #[doc = concat!(" let mut obj = ", stringify!(#struct_name), "::default().", stringify!(#setter_name), "(value);")]
                                    #[doc = concat!(" let value = obj.", stringify!(#take_name), "();")]
                                    #[doc = concat!(" // obj.", stringify!(#field_access), " is now None")]
                                    #[doc = " ```"]
                                    #[doc = ""]
                                    #[doc = " ---"]
                                    #[doc = " *Generated by `aksr` - Builder pattern macro*"]
                                    #getter_inline
                                    #getter_visibility fn #take_name(&mut self) -> Option<#inner> {
                        ...
                        // Whitelist types for which we also generate take_* (they usually implement Default)
                        let name = ident.to_string();
                        let allow_take = matches!(
                            name.as_str(),
                            "Vec"
                                | "String"
                                | "HashMap"
                        ...
                        if allow_take {
                            quote! {
                                ...
                                #[doc = " # Returns"]
                                #[doc = ""]
                                #[doc = concat!(" Returns the owned value of type ", stringify!(#field_type), ".")]
                                #[doc = ""]
                                #[doc = " # Example"]
                                #[doc = ""]
                                #[doc = " ```ignore"]
                                #[doc = concat!(" let obj = ", stringify!(#struct_name), "::default().", stringify!(#setter_name), "(value);")]
                                #[doc = concat!(" let value = obj.", stringify!(#into_name), "();")]
                                #[doc = " // obj is now consumed and cannot be used"]
                                #[doc = " ```"]
                                #[doc = ""]
                                #[doc = " ---"]
                                #[doc = " *Generated by `aksr` - Builder pattern macro*"]
                                #getter_inline
                                #getter_visibility fn #into_name(self) -> #field_type {
                                    self.#field_access
                                }

                                #[doc = concat!(" Takes the `", stringify!(#field_access), "` field and replaces it with `Default::default()`.")]
                                #[doc = ""]
                                #[doc = " This method moves the value out and replaces it with the default value, allowing you to continue using the struct."]
                                #[doc = ""]
                                #[doc = " # Returns"]
                                #[doc = ""]
                                #[doc = concat!(" Returns the owned value of type ", stringify!(#field_type), ".")]
                                ...
                                #[doc = " # Note"]
                                #[doc = ""]
                                #[doc = concat!(" Requires the field type ", stringify!(#field_type), " to implement Default. Prefer this when you want to keep using the instance.")]
                                #[doc = ""]
                                #[doc = " # Example"]
                                #[doc = ""]
                                #[doc = " ```ignore"]
                                #[doc = concat!(" let mut obj = ", stringify!(#struct_name), "::default().", stringify!(#setter_name), "(value);")]
                                #[doc = concat!(" let value = obj.", stringify!(#take_name), "();")]
                                #[doc = concat!(" // obj.", stringify!(#field_access), " is now set to Default::default()")]
                                #[doc = " ```"]
                                #[doc = ""]
                                #[doc = " ---"]
                                #[doc = " *Generated by `aksr` - Builder pattern macro*"]
                                #getter_inline
                                #getter_visibility fn #take_name(&mut self) -> #field_type
                                where
                                    #field_type: ::core::default::Default,
                                {
                                    ::core::mem::take(&mut self.#field_access)
                                }
                            }
                        } else {
                            quote! {
            ...
            // Reference fields: cannot move out meaningfully; only provide into_* moving reference value
            Type::Reference(_) => {
                quote! {
                    #[doc = concat!(" Consumes `self` and returns the `", stringify!(#field_access), "` field.")]
                    #[doc = ""]
                    #[doc = " This method moves the reference out of the struct."]
                    #[doc = ""]
                    #[doc = " # Returns"]
                    #[doc = ""]
                    #[doc = concat!(" Returns the reference of type ", stringify!(#field_type), ".")]
                    #[doc = ""]
                    #[doc = " # Example"]
                    #[doc = ""]
                    #[doc = " ```ignore"]
            ...
            // Arrays, tuples, others owned types
            _ => {
                quote! {
                    #[doc = concat!(" Consumes `self` and returns the `", stringify!(#field_access), "` field.")]
                    #[doc = ""]
